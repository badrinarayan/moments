...
% Plot the performance profile
	...
	t = linspace(1,t_max);
	plot(...
				t,arrayfun(@(x)sum(r_sast<x)/480,t),'k-',...
				t,arrayfun(@(x)sum(r_ast<x)/480,t),'k--',...
				t,arrayfun(@(x)sum(r_cadzow<x)/480,t),'k-.'...
	);
	axis([1 t_max 0 1])
	[h,childObjs] = legend('        AST','        DAST', '        Cadzow','Location','SouthEast');
	hx = xlabel('\beta');
	hy = ylabel('P(\beta)');
	legend(gca,'boxoff');
	...
	for lineIdx = 1:length(xCoords),
		...
		set(lineObjs(lineIdx),'LineWidth',1.5)
	end
